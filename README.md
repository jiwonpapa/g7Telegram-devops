...
> 공개 베타 `v0.6.1-beta.6`: 실제 서버에 설치할 수 있지만 아직 `1.0` 정식판은 아닙니다.
...
책임 제한 고지와 초기설정 진행 여부를 확인한 뒤 다음 항목만 입력합니다.
...
- `원격 서버 재시작`: 기본값은 `N`입니다. 필요한 서버에서만 `Y`로 켭니다.
...
g7telegram-devops_0.6.1-beta.6_amd64.deb: OK
[apt 패키지 설치 로그]
...
서버 재시작은 웹서비스와 Telegram Agent를 함께 중단합니다.
Telegram 원격 서버 재시작 기능을 사용하시겠습니까? [y/N] n
설정 완료: my-vps
웹 상태 검사: https://example.com/
Telegram 서버 재시작: 사용 안 함
...
Agent health: PASS
Installed g7telegram-devops_0.6.1-beta.6_amd64.deb
```
...
설치 후 서버 재시작 메뉴만 켜거나 끌 때는 전체 초기설정을 반복하지 않습니다.
...
활성화하면 `메뉴 → 설정 → ⚡ 전원 관리`가 나타납니다. 이 명령은 Bot token·서버 이름·웹 주소·owner를 변경하지 않습니다.
...
| 선택형 서버 재시작 | 로컬에서 켠 경우에만 60초 단회 확인문구를 직접 입력해 전체 서버 재부팅 |
...
- 서버 재시작: 기본 비활성, root 소유 허용파일, 60초 단회 확인문구와 고정된 `systemctl reboot`만 허용
- 네트워크: inbound port 없이 Telegram API로 outbound HTTPS만 사용
- 상태 저장: 로컬 SQLite, 감사로그와 알림 outbox 크기 제한

현재 `.deb`와 체크섬은 같은 GitHub Release에서 제공되며 독립 서명은 아직 없습니다. 공개 베타에서는 중요한 서버에 적용하기 전에 비핵심 서비스 재시작과 장애·복구 알림을 확인하십시오. 원격 서버 재시작은 VPS 자체가 멈추는 장애를 감지하거나 복구하지 못하며, 클라우드 콘솔을 대신하지 않습니다. 자세한 내용은 [보안 검토 보고서](docs/SECURITY_REVIEW.md)와 [검증 기준](docs/VERIFICATION.md)을 참고하십시오.
