...
> 공개 베타: 실제 서버에 설치할 수 있지만 아직 `1.0` 정식판은 아닙니다. 중요한 서버에서는 설치 후 재시작·장애 알림 검증을 먼저 수행하십시오.
...
| 서비스 상태 | 웹서버, PHP-FPM, DB, 캐시, G7/Laravel 관련 systemd 서비스 자동 탐지·분류 |
...
| Telegram 인증 | 개인채팅, 숫자 user/chat ID, 5분 단회 연결코드, 단일 owner |
...
2. Telegram의 `@BotFather`에서 `/newbot`으로 이 VPS 전용 Bot을 만들고 token을 복사합니다.
...
공개 베타에서는 설치 스크립트를 먼저 내려받아 확인한 뒤 실행하는 방식을 권장합니다.
...
2. GitHub Latest Release에서 `.deb`와 `SHA256SUMS`를 내려받습니다.
...
태그만 있고 Release 산출물이 없는 버전은 설치되지 않습니다. 기본 명령은 항상 GitHub에 실제 공개된 Latest Release를 설치합니다.
...
아래 `0.5.4`는 명령 형식 예시입니다. 실제 사용할 공개 Beta Release 버전으로 바꾸십시오.

```bash
curl -fsSL https://raw.githubusercontent.com/jiwonpapa/g7Telegram-devops/main/scripts/install.sh \
  | sudo G7TG_VERSION=0.5.4 sh
```
...
현재 UX는 공개 베타에 적합하지만 최종형은 아닙니다. 다음 개선 우선순위는 서비스 24개 초과 시 페이지 이동, 마지막 점검시각, 제한된 `설정` 메뉴입니다.
...
현재 Agent는 기본 60초마다 헬스체크하지만 **문제가 생기거나 복구될 때만** Telegram 알림을 보냅니다. 매시간 `정상` 메시지를 보내는 기능은 아직 없습니다.

매시간 정상 알림은 알림 피로가 크므로 기본 기능으로 권장하지 않습니다. 향후 Telegram `설정` 메뉴에 다음 제한된 선택지만 추가하는 방향이 적절합니다.
...
같은 설치 명령을 다시 실행하면 최신 공개 Release로 업데이트하며 기존 설정과 상태를 유지합니다.
...
- `CODE_ONLY`: Rust format, Clippy, 35개 unit/fixture test, dependency audit, ShellCheck
