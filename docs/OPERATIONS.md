...
Telegram의 서버 상태 화면은 모바일 폭에 맞춘 고정폭 표로 표시합니다. 디스크 마운트 경로는 16자까지 같은 줄에 전부 표시하고 더 긴 경로는 생략하지 않고 다음 줄로 나눕니다. Bot API에서는 사용자의 글자 크기를 직접 변경할 수 없으며 실제 크기는 Telegram 앱의 채팅 글자 설정을 따릅니다.
