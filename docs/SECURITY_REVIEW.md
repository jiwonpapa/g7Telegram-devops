...
검토일: 2026-07-20
대상: `main` v0.6.1-beta.6 source, `.deb` 패키지와 기존 `g7devops` Ubuntu 24.04 운영 검증

## 요약

치명적 취약점은 발견하지 못했습니다. Telegram owner 인증, 비밀값 파일 권한, systemd 격리, exact service allowlist와 선택형 서버 재시작의 다중 승인 구조는 공개 베타에 적절합니다.
...
- 전체 서버 재시작은 기본 비활성이고 로컬 root `setup` 또는 `g7tg power enable`, root 소유 `0640` 허용파일, owner 전용 60초 단회 확인문구를 모두 통과해야 합니다. sudoers는 인자를 받지 않는 정확한 `g7tg-exec reboot` 하나만 허용하고 helper는 고정된 `systemctl reboot`만 실행합니다.
...
전체 서버 재시작은 패키지 권한·기본 비활성·확인문구 소비·새 boot ID 판정까지 자동 검증했지만, 운영 중인 `g7devops`를 임의로 재부팅하지 않았습니다. 정식판 전 폐기 가능한 VPS에서 승인, 재부팅, 부팅 후 완료 알림의 왕복 증거가 필요합니다.
...
- Telegram 계정과 Bot token이 동시에 탈취되면 허용된 서비스 작업과, 로컬에서 별도로 켜 둔 서버 재시작이 악용될 수 있습니다. 따라서 중요한 서버에서는 전체 재시작을 꺼 둔 상태가 기본 권고입니다.
