...
검토일: 2026-07-20
대상: `main` v0.6.1-beta.5 source, `.deb` 패키지와 기존 `g7devops` Ubuntu 24.04 운영 검증
...
- 전체 서버 재시작은 기본 비활성이고 로컬 root `setup`, root 소유 `0640` 허용파일, owner 전용 60초 단회 확인문구를 모두 통과해야 합니다. sudoers는 인자를 받지 않는 정확한 `g7tg-exec reboot` 하나만 허용하고 helper는 고정된 `systemctl reboot`만 실행합니다.
