...
검토일: 2026-07-20
대상: `main` v0.6.0 source, `.deb` 패키지, `g7devops` Ubuntu 24.04 운영 인스턴스

## 요약

치명적 취약점은 발견하지 못했습니다. Telegram owner 인증, 비밀값 파일 권한, systemd 격리, exact service allowlist, 단회 재승인 구조는 공개 베타에 적절합니다.

다만 root로 설치되는 공개 제품의 배포 산출물이 별도 서명되지 않았으므로 공급망 보안은 정식 출시 기준에 미달합니다. pairing code 강화, 공개 URL 제한, 보안 제보 경로도 정식판 전에 보완하는 것이 좋습니다.
...
설치 스크립트는 `.deb`와 `SHA256SUMS`를 같은 GitHub Release에서 받아 비교합니다. (`scripts/install.sh:58`) 이 검사는 전송 오류와 우발적 손상은 차단하지만 배포 주체 자체가 침해된 경우를 방어하지 못합니다. 설치 bootstrap도 mutable `main` URL을 사용합니다.
...
### SEC-002 — pairing code가 8자리 hexadecimal이며 실패 제한이 없음

`create_pairing_code_with_mode`는 UUID 앞 8자리, 약 32-bit 값을 5분 기본 연결코드로 사용합니다. (`crates/g7tg-agent/src/storage.rs:207`) 비교는 constant-time이고 유효시간과 단회 소비가 적용되지만, 공개 Bot에 대한 반복 입력을 별도로 제한하지 않습니다.

현재 조건에서 현실적인 추측 성공 가능성은 낮지만 root 관리 Agent의 owner 등록 경계이므로 정식판에서는 강화하는 편이 안전합니다.

권고:

- 최소 16자리 이상의 무작위 코드 또는 전체 128-bit token 사용
- chat/user 단위 실패 횟수 제한과 짧은 backoff
- 실패 시 code 자체를 로그에 남기지 않는 현재 정책 유지
